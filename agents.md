...
24. Codex 聊天界面新增“同步终端”按钮：点击时只读取同一终端会话的 xterm 当前屏幕快照，raw buffer 只作兜底，不向 PTY 写任何输入。若存在 running Codex 回复，继续复用 activePrompt 的实时同步逻辑刷新当前气泡；若没有 active reply，才允许手动路径用 `findLatestCodexPromptText` / `terminalOutputToLatestCodexTurnLiveText` 从当前终端内容主动寻找最新 Codex 用户输入锚点并补建或更新本轮 Codex 聊天消息。实时 streaming 路径仍禁止无锚点整屏 fallback。`terminalMarkdown.test.ts` 覆盖最新 prompt 提取和 idle prompt 忽略，`codexChat.e2e.mjs` 覆盖按钮不写终端、重复同步不重复追加、running 回复复用当前气泡。
